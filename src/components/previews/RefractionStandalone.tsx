...
import { 
  Eye, CheckCircle2, ChevronDown, ChevronUp, ChevronRight, ClipboardList, Clock, 
  Crosshair, User, X, ShieldCheck, Stethoscope, Activity, MapPin, Calendar as CalendarIcon,
  LogOut, Users, Settings, Info
} from "lucide-react";
...
import { getPatientAgeString, cn } from "@/lib/utils";
...
function LocalTopHeader() {
  ...
  return (
    <header className="h-16 md:h-20 border-b border-orange-200/50 bg-white/85 backdrop-blur-xl flex items-center justify-between px-4 md:px-8 shrink-0 shadow-sm z-50 sticky top-0">
      ...
      <div className="flex items-center gap-3 md:gap-6">
        <div className="flex items-center gap-3 pl-2 pr-3 py-1.5 rounded-full border border-slate-200 bg-slate-50/50">
          <div className="w-9 h-9 rounded-full flex items-center justify-center bg-gradient-to-tr from-orange-500 to-rose-500 text-white shadow-lg shadow-orange-200">
      ...
      <div className="px-4 py-4 border-b border-slate-200 bg-white shrink-0">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            ...
            <Badge className="bg-orange-50 text-orange-600 border-0 text-[11px]">{mockPatients.length}</Badge>
          </div>
        </div>

        <div className="flex gap-1 p-0.5 bg-slate-100 rounded-lg">
          <div className="flex-1 px-1 py-1.5 text-[9px] font-black uppercase tracking-wider rounded-md text-center bg-orange-600 text-white shadow-sm cursor-default">
            All
          </div>
        ...
        {mockPatients.map((patient) => (
          <div
            ...
            className={cn(
              ...
              selectedPatientId === patient.id ? "bg-orange-50/40 border-l-orange-600" : "border-l-transparent"
            )}
          >
...
function DiagnosticCard({
  ...
  return (
    <Card className="border border-slate-200 rounded-none shadow-sm overflow-hidden bg-white">
      <div className="px-5 py-3 flex items-center justify-between border-l-4 border-l-orange-600 bg-orange-50/40">
        <div className="flex items-center gap-3 relative">
          <Icon className="w-4 h-4 text-orange-600" />
          <h3 className="text-xs font-black uppercase tracking-wider text-slate-800">{title}</h3>
        </div>
        <div className="flex items-center gap-3">
          {badge && <Badge variant="outline" className="border-orange-300 text-orange-600 text-[8px] font-black uppercase tracking-widest rounded-none bg-white">{badge}</Badge>}
    ...
    <div className="flex flex-col h-full w-full bg-slate-50 overflow-hidden">
      ...
      <div className="flex-1 flex overflow-hidden p-3 gap-3">
        <div className="flex-1 bg-white rounded-3xl border border-slate-200 overflow-hidden flex shadow-sm">
          ...
          <div className="flex-1 flex flex-col min-h-0 overflow-y-auto bg-orange-50/15 relative [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
            ...
            <div className="p-6 lg:p-8 space-y-6 relative z-10 flex-1">
              ...
              <div className="bg-white border border-slate-200 px-6 py-4 flex flex-col sm:flex-row items-start sm:items-center justify-between rounded-xl shadow-sm gap-4">
                <div className="flex items-center gap-4">
                  <div className="bg-gradient-to-br from-orange-500 to-orange-600 text-white p-3 rounded-lg shrink-0 shadow-lg shadow-orange-200">
                    <Eye className="w-6 h-6" />
                  </div>
                  <div>
                    <div className="flex items-center gap-2">
                      <h2 className="text-lg font-black text-slate-900 uppercase tracking-tight">{activePatient.name}</h2>
                      <Badge className="bg-orange-600 text-white text-[10px] px-2 font-mono rounded-full font-bold">MR-{activePatient.mrNumber}</Badge>
                    ...
                    <div className="flex items-center gap-2 text-[10px] font-bold text-slate-400 uppercase tracking-wider mt-1">
                      ...
                      <span>•</span>
                      <span className="text-orange-600 font-bold">Diagnostic Phase</span>
                    </div>
                  </div>
                </div>
              ...
              <div className="space-y-6 max-w-5xl mx-auto">
                  ...
                  <div className="space-y-4">
                    <div className="flex flex-wrap gap-2">
                      {["Diabetes", "Hypertension"].map((cond) => (
                        <span key={cond} className="px-3 py-1.5 bg-orange-50 text-orange-600 border border-orange-200 text-xs font-black uppercase tracking-wider">
                          {cond}
                        </span>
                      ))}
                    </div>
                ...
                <div className="border border-slate-200 bg-white p-6 flex flex-col sm:flex-row items-center justify-between gap-4">
                  <div className="flex items-center gap-3">
                    <div className="w-6 h-6 border border-slate-300 rounded bg-slate-50 flex items-center justify-center text-orange-600">
                      <CheckCircle2 className="w-4 h-4 text-orange-600" />
                    </div>
                    <span className="text-xs font-bold text-slate-700 uppercase tracking-wide">Patient values have been verified</span>
                  </div>

                  <Button className="h-11 bg-orange-600 hover:bg-black text-white font-bold uppercase tracking-wider px-8 rounded-none shadow-md gap-2" disabled>
