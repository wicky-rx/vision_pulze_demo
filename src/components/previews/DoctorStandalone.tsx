...
import {
  Stethoscope, Eye, CheckCircle2, ChevronDown, ClipboardList, Clock,
  User, ShieldCheck, Activity, Calendar as CalendarIcon, Users, Plus, Save
} from "lucide-react";
...
function LocalTopHeader() {
  return (
    <header className="h-16 border-b border-orange-200/50 bg-white/85 backdrop-blur-xl flex items-center justify-between px-6 shrink-0 shadow-sm z-50 sticky top-0">
      ...
      <div className="flex items-center gap-3">
        <div className="flex items-center gap-3 pl-2 pr-3 py-1.5 rounded-full border border-slate-200 bg-slate-50/50">
          <div className="w-8 h-8 rounded-full flex items-center justify-center bg-gradient-to-tr from-orange-500 to-rose-500 text-white shadow-md">
      ...
      <div className="px-4 py-4 border-b border-slate-200 bg-white shrink-0">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-xs font-semibold text-slate-800">Live Queue</h3>
            <Badge className="bg-orange-50 text-orange-600 border-0 text-[10px]">{mockPatients.length}</Badge>
          </div>
        </div>
...
function DiagnosticCard({ title, icon: Icon, badge, children }: { title: string; icon: any; badge?: string; children: React.ReactNode }) {
  return (
    <Card className="border border-slate-200 rounded-none shadow-sm overflow-hidden bg-white">
      <div className="px-4 py-2.5 flex items-center justify-between border-l-4 border-l-orange-600 bg-orange-50/40">
        <div className="flex items-center gap-2">
          ...
          <div className="flex-1 flex flex-col min-h-0 overflow-y-auto bg-orange-50/15 relative [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
            ...
            <div className="p-4 lg:p-6 space-y-4 relative z-10 flex-1">
              ...
              <div className="bg-white border border-slate-200 px-5 py-3.5 flex flex-col sm:flex-row items-start sm:items-center justify-between rounded-xl shadow-sm gap-3">
                <div className="flex items-center gap-3">
                  <div className="bg-gradient-to-br from-orange-500 to-orange-600 text-white p-2.5 rounded-lg shrink-0">
                    <Stethoscope className="w-5 h-5" />
                  </div>
                  <div>
                    <div className="flex items-center gap-2 flex-wrap">
                      <h2 className="text-sm font-black text-slate-900 uppercase tracking-tight">{activePatient.name}</h2>
                      <Badge className="bg-orange-600 text-white text-[9px] px-1.5 font-mono rounded-full font-bold">MR-{activePatient.mrNumber}</Badge>
                    ...
                    <div className="flex items-center gap-2 text-[9px] font-bold text-slate-400 uppercase tracking-wider mt-0.5">
                      ...
                      <span>•</span>
                      <span className="text-orange-600 font-bold">Consultation</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-4 max-w-4xl mx-auto">
                ...
                <div className="border border-slate-200 bg-white p-4 flex flex-col sm:flex-row items-center justify-between gap-3">
                  ...
                  <Button className="h-10 bg-orange-600 hover:bg-black text-white font-bold uppercase tracking-wider px-6 rounded-none shadow-sm gap-2" disabled>
