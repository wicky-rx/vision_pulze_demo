...
import { getPatientAgeString, calculateSessionSlot, cn } from "@/lib/utils";
...
export function OpticalStation({ patient, doctors = [] }: { patient?: Patient | null, doctors?: any[] }) {
  ...
  const handleComplete = async () => {
    ...
    try {
      ...
      // Correct API endpoint for optical fulfillment
      const response = await fetch(`${API_BASE_URL}/api/optical/${patient.id}`, {
        ...
        body: JSON.stringify({ 
          ...
          // You could also send lensPower if needed, 
          // but usually it's derived from the clinician's Rx
      ...
      {!consultation ? (
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-6 flex-1 items-start relative z-10">
          ...
          <div className="xl:col-span-2 space-y-6">
            {/* Spectacle Order */}
            <Card className="shadow-sm border-slate-200 rounded-none overflow-hidden">
               <CardHeader className="bg-white p-0 border-b border-slate-100 overflow-hidden">
                <div className="flex items-stretch h-14">
            ...
            {/* Actions */}
            <div className="flex flex-col sm:flex-row justify-end gap-4 pt-8 mt-4 border-t border-slate-200 pb-12">
              <Button 
                variant="outline" 
